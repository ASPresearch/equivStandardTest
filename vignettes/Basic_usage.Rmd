...
crossTabUnrestricted <- crossTabGOIDs4GeneLists (genelist1=gl1, genelist2=gl2,
                                                 ...
                                                 onto=anOnto, GOLevel=GOLev, restricted=restricted)
...
(listOfGOTerms1<-  GOTermsList(shortList, "BP", orgPkg = "org.Hs.eg.db"))
...
listOfGOTerms2 <- dfOfGOTerms2 %>%
  ...
  dplyr::select ("GOALL")
length(unique(listOfGOTerms2[,1]))
```
...
length(ancestorsList1)
length(unique(listOfGOTerms2[,1]))
setdiff(ancestorsList1, listOfGOTerms2[,1])
```
...
```{r eval=FALSE}
library(GOstats)
graphBP1<-makeGOGraph(shortList,"BP", chip="hgu133plus2.db")
length(nodes(graphBP1))
```

```{r plotGOGraph1, out.width = '70%', eval=FALSE}
